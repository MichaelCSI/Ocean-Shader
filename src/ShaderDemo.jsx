...
import { useGLTF, Trail, Float, Environment, Text, Stars } from '@react-three/drei'
...
function Water()
{
    const { bloom, stats, shader } = useControls('Settings', {
        bloom: false,
        stats: false,
        shader: { options: [ 'Normal', 'Night', 'Cursor Color', 'Glow' ] }
    })
    ...
    useFrame((state) => {
        ...
        ocean.current.material.uniforms.uFragShaderMode.value = shaderMap.get(shader)
    })
    ...
    return <>
        ...
        <Float speed={ 3 } floatIntensity={ 0.2 } rotationIntensity={ 0.1 }>
            <Text
                ...
                >
                    {
                        shaderMap.get(shader) === 1 &&
                        "Normal Ocean Shader" ||
                        shaderMap.get(shader) === 2 &&
                        "Hover on the water!" ||
                        shaderMap.get(shader) === 3 &&
                        "Move the mouse around!" ||
                        shaderMap.get(shader) === 4 &&
